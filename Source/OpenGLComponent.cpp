...
void OpenGLComponent::newOpenGLContextCreated()
{
    ...
    fragmentShader =
            ...
            void paintPoint2HalfSlices(float distToPoint, vec2 currentP) {
            ...
            gl_FragColor = mix(white, backgroundColour, (distToPoint - outerRadius) / (outerRadiusBorder - outerRadius));
            else if(distanceToLine(currentP, 0.25) <= lineWidth / 2.0)
            gl_FragColor = white;
            else if(currentP.x < 0.0) {
            if(when_lt(distToPoint, radius) == 1.0)
            gl_FragColor = vec4(sliceRed[0], sliceGreen[0], sliceBlue[0], 1.0);
            else
            gl_FragColor = mix(vec4(sliceRed[0], sliceGreen[0], sliceBlue[0], 1.0), white, (distToPoint - radius) / (outerRadius - radius));
            } else {
            if(when_lt(distToPoint, radius) == 1.0)
            gl_FragColor = vec4(sliceRed[1], sliceGreen[1], sliceBlue[1], 1.0);
            else
            gl_FragColor = mix(vec4(sliceRed[1], sliceGreen[1], sliceBlue[1], 1.0), white, (distToPoint - radius) / (outerRadius - radius));
            }
            ...
            paintPoint2HalfSlices(distToPoint, currentP);
            else
